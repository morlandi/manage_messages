...
def list_available_languages(settings_module):
    ...
    #     if code_prefix(code) != language_code
    # ]
...
def do_auto_translatemessages(apps, languages, fuzzy):
    ...
    for app in apps:
        ...
        app_path = get_app_path(app)
        for language in languages:
            # es: "python po_auto_translate.py frontend/locale/it/LC_MESSAGES/django.po --fuzzy"
            po_file = os.path.join(app_path, "locale", language, "LC_MESSAGES", "django.po")
            po_file = os.path.normpath(po_file)
            command = f'python po_auto_translate.py "{po_file}"'
            if fuzzy:
                command += " --fuzzy"
            run_command(command)
...
def do_installmessages(apps, languages, source_folder):
    ...
    for language in languages:
        ...
        for app in apps:
            ...
            if source_path:
                path = os.path.join(
                    app_path, "locale", language, "LC_MESSAGES", "django"
                )
...
def main():
    ...
    available_languages = list_available_languages(
        config.get("general", "settings_module").strip()
    )
